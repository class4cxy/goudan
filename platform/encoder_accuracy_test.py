...
# 实测校准（DEBOUNCE_US=0 生效后需重新跑一次卷尺校准）
# 上次校准（DEBOUNCE_US=20，40%速）：里程计1528.6mm 实测3150mm → 99 = 204×(1528.6/3150)
# DEBOUNCE_US=0 后捕获率大幅提升，预计值会比 99 大很多，重测后填入
LINES_PER_REV: int = 99
...
# 去抖参数。
# ⚠ DEBOUNCE_US 不能过大：89 RPM×500线×4倍频 → 脉冲间隔仅 5.6μs
#   20μs 延时会漏掉 3+ 个脉冲，导致里程严重低估且随速度变化。
#   设为 0 消除人为延时，仅靠 lgpio 轮询速率做自然滤波。
DEBOUNCE_READS: int = 1    # 去抖读取次数
DEBOUNCE_US:    int = 0    # 去抖延时（微秒）：必须为 0，否则高速漏脉冲
...
# 左右轮速度补偿。
# 40%速实测：R=1593.3mm / L=1464.0mm → right_scale = 1464/1593 = 0.919 → 取 0.92
# DEBOUNCE_US=0 后建议重测确认（但 scale 受速度影响较小，可先用）
CHASSIS_LEFT_SCALE:  float = 1.0
CHASSIS_RIGHT_SCALE: float = 0.92

# ── 测试参数 ──────────────────────────────────────────────────────────────────

# 目标行驶距离（mm）。正数=前进，负数=后退。
# 短距离（<1s）时噪声比例高、测量不稳定；建议 ≥1500mm
TARGET_DISTANCE_MM: float = 1500.0
