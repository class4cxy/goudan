...
# 实测校准（DEBOUNCE_US=3，40%速，1500mm）：里程计1566.9mm 实测1400mm
# 新值 = 99 × (1566.9/1400) = 111（里程计高估 → 增大 lines_per_rev 使每 tick 代表更少距离）
LINES_PER_REV: int = 111
...
# 去抖参数。
# ⚠ DEBOUNCE_US 权衡：脉冲间隔 5.6μs（89 RPM×500线×4倍频）
#   = 0  → 无过滤，EMF 噪声直接计入，右编码器虚增 ticks，物理观测与数据方向相反
#   = 20 → 过长，每次跳变后等 20μs，漏掉 3+ 个真实脉冲，里程随速度漂移
#   = 3  → 折中：可过滤 <3μs 的 EMF 毛刺，同时不丢 5.6μs 周期的真实脉冲
DEBOUNCE_READS: int = 1    # 去抖读取次数
DEBOUNCE_US:    int = 3    # 去抖延时（微秒）
