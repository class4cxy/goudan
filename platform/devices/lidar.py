...
接线说明（LD06 → CP2102 → 树莓派 USB）：
  LD06 P5V  → CP2102 5V（或扩展板 5V）
  LD06 GND  → CP2102 GND（共地）
  LD06 Tx   → CP2102 RXD   ← 只需接收
  LD06 PWM  → 悬空         ← 实测该引脚无法控制电机（内部 PID 锁速 ~600 RPM）

电机控制说明：
  LD06 电机由内部 MCU 的 PID 控制器锁定在约 600 RPM（10 Hz 扫描频率）。
  实测验证：将 GPIO12 硬件 PWM0（25kHz）连接到 LD06 PWM 引脚，
  无论占空比如何变化（0%~100%），电机转速始终维持 597 RPM，不受外部 PWM 控制。
  如需真正停止电机，须在 5V 供电线串联 P-MOSFET 切断电源（硬件改造）。
  软件层面：stop() 关闭串口停止数据流，电机在 LD06 通电期间持续旋转。
...
class Lidar:
    ...
    def stop(self) -> None:
        """停止读取线程并关闭串口。LD06 电机在通电期间持续旋转，无法通过软件停止。"""
