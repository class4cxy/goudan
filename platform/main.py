...
async def _startup():
    ...
    # 超声波传感器已禁用：GPIO20/21 已改接 M4 右后编码器，两者冲突
    # 恢复超声波需先把 M4 编码器迁移到其他空闲引脚
    logger.info("📏 超声波传感器已禁用（GPIO20/21 被 M4 编码器占用）")
...
async def motor_drive(req: MotorDriveRequest):
    """
    运动接口，支持两种模式（通过 .env 切换）：

    编码器/IMU 闭环（默认，DRIVE_SPEED_MM_PER_SEC=0 / DRIVE_TURN_DEG_PER_SEC=0）：
    - distance_mm: 编码器路程积累，达到目标距离停止
    - angle_deg:   IMU 陀螺仪积分，达到目标角度停止

    时间标定控制（编码器 EMF 噪声严重时的临时方案）：
    - DRIVE_SPEED_MM_PER_SEC > 0: duration = target_mm / speed_mm_per_sec
    - DRIVE_TURN_DEG_PER_SEC > 0: duration = target_deg / turn_deg_per_sec
    """
    ...
    try:
        if req.distance_mm is not None:
            ...
            if _drive_speed_mm_s > 0:
                # ── 时间标定控制（编码器 EMF 噪声临时方案）─────────────
                duration_s = target_mm / _drive_speed_mm_s
            ...
            if _turn_deg_s > 0:
                # ── 时间标定转向（IMU 不可用时的临时方案）──────────────
                duration_s = target_deg / _turn_deg_s
                chassis._dispatch(direction, speed)
                await asyncio.sleep(duration_s)
                chassis.stop()
                logger.info(
...
async def teleop_start(req: TeleopStartRequest):
    ...
    安全约束：
      - LiDAR 防撞可通过 lidar_safety_enabled 开关（默认关闭）
      - 开启后才会确保 LiDAR 在线并执行最小安全距离检查（防碰撞）
    """
    if chassis.is_simulation:
        raise HTTPException(status_code=503, detail="底盘处于模拟模式，无法进入遥控")

    _env_lidar_safety_enabled = os.environ.get("TELEOP_LIDAR_SAFETY_ENABLED", "0").strip() in ("1", "true", "True", "yes", "on")
    lidar_safety_enabled = req.lidar_safety_enabled if req.lidar_safety_enabled is not None else _env_lidar_safety_enabled

    if lidar_safety_enabled:
        ...
        if lidar_sensor.device.is_simulation:
            raise HTTPException(status_code=503, detail="激光雷达不可用，拒绝启用 LiDAR 遥控防撞")
...
async def teleop_command(req: TeleopCommandRequest):
    ...
    #  - 前进：检查前向扇区（0°）
    #  - 后退：检查后向扇区（180°）
    if lidar_safety_enabled and abs(throttle) > deadband:
        if lidar_sensor.device.is_simulation:
            chassis.stop()
            async with _teleop_lock:
                _teleop_state["last_input_monotonic"] = time.monotonic()
