...
@dataclass
class SlamConfig:
    # 地图尺寸（增大可减轻轮廓消失：小地图易被新扫描覆盖）
    map_size_pixels: int   = 1000    # 地图分辨率（正方形边长，像素），原 500
    ...
    sigma_xy_mm: float     = 60.0    # 位置不确定度（mm）；慢速移动适中值，原 100 太大易误匹配
    sigma_theta_deg: float = 15.0    # 角度不确定度（度）；原 20 太大，5 太小导致爆炸，15 为折中
...
def config_from_env() -> SlamConfig:
    ...
    return SlamConfig(
        ...
        sigma_xy_mm=float(os.environ.get("SLAM_SIGMA_XY_MM", "60")),
        sigma_theta_deg=float(os.environ.get("SLAM_SIGMA_THETA_DEG", "15")),
    ...
    def start_mapping(self) -> bool:
        ...
        with self._lock:
            self._slam = RMHC_SLAM(
                ...
                max_search_iter=self._cfg.max_search_iter,
            )
