...
| GPIO 20 | Pin 38 | HC-SR04 Trig | 超声波测距 | `platform/devices/ultrasonic.py` | 默认 Trig 引脚 |
| GPIO 21 | Pin 40 | HC-SR04 Echo | 超声波测距 | `platform/devices/ultrasonic.py` | 默认 Echo 引脚（注意 5V 电平问题） |
...
| 🟡 中 | GPIO 21 | 当前作为 HC-SR04 Echo 输入；HC-SR04 Echo 为 5V，若扩展板无电平转换会损伤 GPIO | 优先使用扩展板已带转换接口；若直连树莓派需分压（例如 1k+2k） |
| 🟡 中 | GPIO 20/21 | 这两个引脚兼 SPI1 MOSI/CLK，若未来接 SPI1 设备会与超声波冲突 | 若需启用 SPI1，先将超声波迁移到其他空闲 GPIO 并同步更新环境变量 |
...
| GPIO 20 | Pin 38 | SPI1 MOSI | （已被超声波 Trig 占用，默认不建议复用） |
| GPIO 21 | Pin 40 | SPI1 CLK | （已被超声波 Echo 占用，默认不建议复用） |
...
GPIO11/SCLK (23) (24) GPIO8/CE0   ← 空闲(SPI0 CLK) | 空闲
  ...
  GPIO13/PWM1 (33) (34) GND         ← 舵机 Tilt（云台垂直，⚠️扩展板GP13已引出）
       ...
       GPIO26 (37) (38) GPIO20      ← M2 IN2（右前轮） | 超声波 Trig
          GND (39) (40) GPIO21      ← 超声波 Echo
